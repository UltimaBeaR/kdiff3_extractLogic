   ...
   if(m_maxTextWidth < 0)
   {
       ...
       for(mlIt = m_pMergeDataObj->m_mergeLineList.begin(); mlIt != m_pMergeDataObj->m_mergeLineList.end(); ++mlIt)
       {
           ...
           for(melIt = ml.mergeEditLineList.begin(); melIt != ml.mergeEditLineList.end(); ++melIt)
           {
               ...
               QString s = mel.getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
...
void MergeResultWindow::go(e_Direction eDir, e_EndPoint eEndPoint)
{
   ...
   MergeLineList::iterator i = m_pMergeDataObj->m_currentMergeLineIt;
   bool bSkipWhiteConflicts = !m_pOptions->m_bShowWhiteSpace;
   if(eEndPoint == eEnd)
   {
       if(eDir == eUp)
           i = m_pMergeDataObj->m_mergeLineList.begin(); // first mergeline
       else
...
{
   bool bSkipWhiteConflicts = !m_pOptions->m_bShowWhiteSpace;
   if(m_pMergeDataObj->m_mergeLineList.empty()) return false;
   MergeLineList::iterator i = m_pMergeDataObj->m_currentMergeLineIt;
   if(i == m_pMergeDataObj->m_mergeLineList.begin()) return false;
   do
   {
       --i;
       if(i->bDelta && !checkOverviewIgnore(i) && !(bSkipWhiteConflicts && i->bWhiteSpaceConflict)) return true;
   } while(i != m_pMergeDataObj->m_mergeLineList.begin());
...
bool MergeResultWindow::isDeltaBelowCurrent()
{
   ...
   MergeLineList::iterator i = m_pMergeDataObj->m_currentMergeLineIt;
   if(i != m_pMergeDataObj->m_mergeLineList.end())
   {
       ++i;
       for(; i != m_pMergeDataObj->m_mergeLineList.end(); ++i)
       {
           if(i->bDelta && !checkOverviewIgnore(i) && !(bSkipWhiteConflicts && i->bWhiteSpaceConflict)) return true;
       }
   }
...
{
   setFastSelector(m_pMergeDataObj->m_currentMergeLineIt);
}
...
void MergeResultWindow::showNrOfConflicts()
{
   ...
   QString totalInfo;
   if(m_pMergeDataObj->m_pTotalDiffStatus->isBinaryEqualAB() && m_pMergeDataObj->m_pTotalDiffStatus->isBinaryEqualAC())
       totalInfo += i18n("All input files are binary equal.");
   else if(m_pMergeDataObj->m_pTotalDiffStatus->isTextEqualAB() && m_pMergeDataObj->m_pTotalDiffStatus->isTextEqualAC())
       totalInfo += i18n("All input files contain the same text.");
   else
   {
       if(m_pMergeDataObj->m_pTotalDiffStatus->isBinaryEqualAB())
           totalInfo += i18n("Files %1 and %2 are binary equal.\n", i18n("A"), i18n("B"));
       else if(m_pMergeDataObj->m_pTotalDiffStatus->isTextEqualAB())
           totalInfo += i18n("Files %1 and %2 have equal text.\n", i18n("A"), i18n("B"));
       if(m_pMergeDataObj->m_pTotalDiffStatus->isBinaryEqualAC())
           totalInfo += i18n("Files %1 and %2 are binary equal.\n", i18n("A"), i18n("C"));
       else if(m_pMergeDataObj->m_pTotalDiffStatus->isTextEqualAC())
           totalInfo += i18n("Files %1 and %2 have equal text.\n", i18n("A"), i18n("C"));
       if(m_pMergeDataObj->m_pTotalDiffStatus->isBinaryEqualBC())
           totalInfo += i18n("Files %1 and %2 are binary equal.\n", i18n("B"), i18n("C"));
       else if(m_pMergeDataObj->m_pTotalDiffStatus->isTextEqualBC())
           totalInfo += i18n("Files %1 and %2 have equal text.\n", i18n("B"), i18n("C"));
   }
...
void MergeResultWindow::setFastSelector(MergeLineList::iterator i)
{
   if(i == m_pMergeDataObj->m_mergeLineList.end())
       return;
   m_pMergeDataObj->m_currentMergeLineIt = i;
   ...
   for(mlIt = m_pMergeDataObj->m_mergeLineList.begin(); mlIt != m_pMergeDataObj->m_mergeLineList.end(); ++mlIt)
   {
       if(mlIt == m_pMergeDataObj->m_currentMergeLineIt)
           break;
       line1 += mlIt->mergeEditLineList.size();
   }

   int nofLines = m_pMergeDataObj->m_currentMergeLineIt->mergeEditLineList.size();
...
{
    if(m_pMergeDataObj->m_currentMergeLineIt == m_pMergeDataObj->m_mergeLineList.end())
        return;

    setModified();

    m_pMergeDataObj->choose(selector);
...
void MergeResultWindow::slotMergeHistory()
{
   ...
   m_pMergeDataObj->m_pDiff3LineList->findHistoryRange(QRegExp(m_pOptions->m_historyStartRegExp), m_pMergeDataObj->m_pldC != nullptr, iD3LHistoryBegin, iD3LHistoryEnd, d3lHistoryBeginLineIdx, d3lHistoryEndLineIdx);

   if(iD3LHistoryBegin != m_pMergeDataObj->m_pDiff3LineList->end())
   {
       // Now collect the historyMap information
       HistoryMap historyMap;
       std::list<HistoryMap::iterator> hitList;
       if(m_pMergeDataObj->m_pldC == nullptr)
       {
       ...
       if(m_pOptions->m_maxNofHistoryEntries == -1)
       {
           ...
           if(bHistoryMergeSorting)
           {
               while(!historyMap.empty())
               {
                   ...
                   if(hMapIt->second.staysInPlace(m_pMergeDataObj->m_pldC != nullptr, iD3LHistoryEnd))
       ...
       if(i != iMLLEnd)
       {
           ...
           {
               iMLLStart->join(*i);
               i = m_pMergeDataObj->m_mergeLineList.erase(i);
           }
       }
       iMLLStart->mergeEditLineList.clear();
       // Now insert the complete history into the first MergeLine of the history
       iMLLStart->mergeEditLineList.push_back(MergeEditLine(iD3LHistoryBegin, m_pMergeDataObj->m_pldC == nullptr ? e_SrcSelector::B : e_SrcSelector::C));
       QString lead = Utils::calcHistoryLead(iD3LHistoryBegin->getString(e_SrcSelector::A));
       MergeEditLine mel(m_pMergeDataObj->m_pDiff3LineList->end());
       mel.setString(lead);
       ...
       if(bHistoryMergeSorting)
       {
           ...
           for(hmit = historyMap.rbegin(); hmit != historyMap.rend(); ++hmit)
           {
               ...
               MergeEditLineList& mell = hme.choice(m_pMergeDataObj->m_pldC != nullptr);
               if(!mell.empty())
       ...
       else
       {
           ...
           {
               QString lastLineOfStart = iMLLStart->mergeEditLineList.back().getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
               QString firstLineOfEnd = iMLLEnd->mergeEditLineList.front().getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
   ...
   for(i = m_pMergeDataObj->m_mergeLineList.begin(); i != m_pMergeDataObj->m_mergeLineList.end(); ++i)
   {
       if(i->bConflict)
       {
           ...
           if(vcsKeywords.exactMatch(id3l->getString(e_SrcSelector::A)) &&
              ...
              (m_pMergeDataObj->m_pldC == nullptr || vcsKeywords.exactMatch(id3l->getString(e_SrcSelector::C))))
           {
               MergeEditLine& mel = *i->mergeEditLineList.begin();
               mel.setSource(m_pMergeDataObj->m_pldC == nullptr ? e_SrcSelector::B : e_SrcSelector::C, false);
...
{
   if(m_pMergeDataObj->m_pldC == nullptr || m_pMergeDataObj->m_mergeLineList.size() <= 1)
       return true;
...
MergeLineList::iterator MergeResultWindow::splitAtDiff3LineIdx(int d3lLineIdx)
{
   ...
   for(i = m_pMergeDataObj->m_mergeLineList.begin(); i != m_pMergeDataObj->m_mergeLineList.end(); ++i)
   {
       ...
       else if(i->d3lLineIdx > d3lLineIdx)
       {
           ...
           ++i;
           return m_pMergeDataObj->m_mergeLineList.insert(i, newML);
       }
   }
   // The split must be in the previous MergeLine
   --i;
...
{
   MergeLineList::iterator i;
   MergeLineList::iterator iMLLStart = m_pMergeDataObj->m_mergeLineList.end();
   MergeLineList::iterator iMLLEnd = m_pMergeDataObj->m_mergeLineList.end();
   for(i = m_pMergeDataObj->m_mergeLineList.begin(); i != m_pMergeDataObj->m_mergeLineList.end(); ++i)
   {
       MergeLine& ml = *i;
   ...
   bool bJoined = false;
   for(i = iMLLStart; i != iMLLEnd && i != m_pMergeDataObj->m_mergeLineList.end();)
   {
       if(i == iMLLStart)
       {
           ++i;
...
void MergeResultWindow::setPaintingAllowed(bool bPaintingAllowed)
{
   ...
   if(!bPaintingAllowed)
   {
       m_pMergeDataObj->m_currentMergeLineIt = m_pMergeDataObj->m_mergeLineList.end();
       reset();
   }
   else
       update();
...
void MergeResultWindow::paintEvent(QPaintEvent*)
{
           ...
           else
           {
               ...
               for(melIt = ml.mergeEditLineList.begin(); melIt != ml.mergeEditLineList.end(); ++melIt)
               {
                   if(line >= m_firstLine && line <= lastVisibleLine)
                   {
                       ...
                       if(mlIt == m_pMergeDataObj->m_currentMergeLineIt) rangeMark |= 4; // Mark of the current line

                       QString s;
                       s = mel.getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
   ...
   if(m_bCursorOn && hasFocus() && m_cursorYPos >= m_firstLine)
   {
       ...
       QString str = m_pMergeDataObj->getString(m_cursorYPos);
       QTextLayout textLayout(str, font(), this);
...
void MergeResultWindow::updateSourceMask()
{
   int srcMask = 0;
   int enabledMask = 0;
   if(!hasFocus() || m_pMergeDataObj->m_pDiff3LineList == nullptr || !updatesEnabled() || m_pMergeDataObj->m_currentMergeLineIt == m_pMergeDataObj->m_mergeLineList.end())
   {
       srcMask = 0;
       enabledMask = 0;
   }
   ...
   {
       enabledMask = m_pMergeDataObj->m_pldC == nullptr ? 3 : 7;
       MergeLine& ml = *m_pMergeDataObj->m_currentMergeLineIt;
...
void MergeResultWindow::mousePressEvent(QMouseEvent* e)
{
   ...
   QString s = m_pMergeDataObj->getString(line);
   QTextLayout textLayout(s, font(), this);
   ...
   if((bLMB && (e->x() < xOffset)) || bRMB) // Fast range selection
   {
       ...
       int l = 0;
       MergeLineList::iterator i = m_pMergeDataObj->m_mergeLineList.begin();
       for(i = m_pMergeDataObj->m_mergeLineList.begin(); i != m_pMergeDataObj->m_mergeLineList.end(); ++i)
       {
           if(l == line)
               break;
...
void MergeResultWindow::keyPressEvent(QKeyEvent* e)
{
   ...
   MergeEditLineList::iterator melIt;
   if(!m_pMergeDataObj->calcIteratorFromLineNr(y, mlIt, melIt))
   {
   ...
   QString str = melIt->getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
   int x = m_cursorXPos;
   ...
   switch(e->key())
   {
       ...
       case Qt::Key_Delete: {
           ...
           if(x >= str.length())
           {
               if(y < m_nofLines - 1)
               {
                   ...
                   if(m_pMergeDataObj->calcIteratorFromLineNr(y + 1, mlIt1, melIt1) && melIt1->isEditableText())
                   {
                       QString s2 = melIt1->getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
       ...
       case Qt::Key_Backspace: {
           ...
           if(x == 0)
           {
               if(y > 0)
               {
                   ...
                   if(m_pMergeDataObj->calcIteratorFromLineNr(y - 1, mlIt1, melIt1) && melIt1->isEditableText())
                   {
                       QString s1 = melIt1->getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
       ...
       case Qt::Key_Enter: {
           ...
           { // calc last indentation
               ...
               for(;;)
               {
                   const QString s = melIt1->getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
                   if(!s.isEmpty())
                   {
                       int i;
                   ...
                   {
                       if(mlIt1 == m_pMergeDataObj->m_mergeLineList.begin()) break;
                       --mlIt1;
   ...
   str = m_pMergeDataObj->calcIteratorFromLineNr(y, mlIt, melIt)
             ? melIt->getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC)
             : QString();
   ...
   for(mlIt = m_pMergeDataObj->m_mergeLineList.begin(); mlIt != m_pMergeDataObj->m_mergeLineList.end(); ++mlIt)
   {
       ...
       for(melIt = ml.mergeEditLineList.begin(); melIt != ml.mergeEditLineList.end(); ++melIt)
       {
           ...
           if(m_selection.lineWithin(line))
           {
               ...
               {
                   const QString str = mel.getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
                                        ...
                                        MergeLineList::iterator& mlIt, MergeEditLineList::iterator& melIt)
{
   if(m_selection.selectionContainsData())
   {
       ...
       s = melIt->getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
       x = m_cursorXPos;
       return true;
   }
...
void MergeResultWindow::deleteSelection()
{
   ...
   for(mlIt = m_pMergeDataObj->m_mergeLineList.begin(); mlIt != m_pMergeDataObj->m_mergeLineList.end(); ++mlIt)
   {
       ...
       for(melIt = ml.mergeEditLineList.begin(); melIt != ml.mergeEditLineList.end();)
       {
           ...
           if(mel.isEditableText() && m_selection.lineWithin(line))
           {
               QString lineString = mel.getString(m_pMergeDataObj->m_pldA, m_pMergeDataObj->m_pldB, m_pMergeDataObj->m_pldC);
...
bool MergeResultWindow::saveDocument(const QString& fileName, QTextCodec* pEncoding, e_LineEndStyle eLineEndStyle)
{
   ...
   if(m_pOptions->m_bDmCreateBakFiles && file.exists())
   {
    ...
    void* buffer;
    qint64 bufferLength;
    if (!m_pMergeDataObj->getOutFileData(pEncoding, eLineEndStyle, buffer, bufferLength)) {
        return false;
    }

   bool bSuccess = file.writeFile(buffer, bufferLength);
   if(!bSuccess)
   {
...
bool MergeResultWindow::findString(const QString& s, LineRef& d3vLine, int& posInLine, bool bDirDown, bool bCaseSensitive)
{
   ...
   {
       QString line = m_pMergeDataObj->getString(it);
       if(!line.isEmpty())
       {
...
void MergeResultWindow::setSelection(int firstLine, int startPos, int lastLine, int endPos)
{
   if(lastLine >= getNofLines())
   {
       ...
       QString s = m_pMergeDataObj->getString(lastLine);
       endPos = s.length();
   }
   m_selection.reset();
