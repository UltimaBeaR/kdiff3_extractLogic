...
// TODO: я хочу попробовать разделить эти опции на данные и визуальные вещи.
// То есть попробовать выделить из этого всего данные и хранить их в виде отдельных опций, но при этом основной объект опций все равно будет использовать внутри штуки с данными, чтобы не править весь код где есть зависимость на опции.
// далее в местах где нужны только данные (по сути весь мой итоговый зарефакторенный код мержа и диффов) - там переделать чтобы юзался объект опций-данных вместо просто опций
// Также тут есть всякие опции-данные касаемые мержа директорий и тому подобное - то что мне не нужно в итоге. Это тоже оставляю в основных опциях и не тяну в свой объект с данными
